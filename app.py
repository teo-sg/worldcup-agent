...
<style>
    ...
    .tag-green  { background:#00c85322; color:#00c853; border:1px solid #00c85344; border-radius:4px; padding:2px 8px; font-size:11px; font-weight:700; }
    .tag-cyan   { background:#00e5ff22; color:#00e5ff; border:1px solid #00e5ff44; border-radius:4px; padding:2px 8px; font-size:11px; font-weight:700; }
    .tag-red    { background:#ff3d0022; color:#ff3d00; border:1px solid #ff3d0044; border-radius:4px; padding:2px 8px; font-size:11px; font-weight:700; }
    .tag-gold   { background:#ffd70022; color:#ffd700; border:1px solid #ffd70044; border-radius:4px; padding:2px 8px; font-size:11px; font-weight:700; }
</style>
...
with tab_wc:
    ...
    st.warning(
        "⚠️ 2026 FIFA 월드컵 조 추첨은 공식 미실시 상태입니다. "
        "본선 경기 배당은 대회 개막 후 자동 수신됩니다. "
        "현재는 국제 A매치 / 친선경기 배당을 분석합니다."
    )
    ...
    if manual_mode:
        ...
        if not games:
            st.info("버튼을 눌러 배당 피드를 불러오세요. 경기가 없는 날은 데이터가 비어 있습니다.")
        else:
            # 경기 목록 표시
...
with tab_kbo:
    ...
    if not kbo_games:
        st.info("버튼을 눌러 KBO 배당 피드를 불러오세요. 시즌 외 기간이거나 경기가 없는 날은 비어 있습니다.")
    else:
